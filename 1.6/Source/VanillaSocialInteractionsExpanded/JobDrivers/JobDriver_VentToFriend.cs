	...
	public class JobDriver_VentToFriend : JobDriver
	{
		...
		public override IEnumerable<Toil> MakeNewToils()
		{
			...
			toil.tickAction = delegate
			{
				...
				Map map = pawn.Map;
				if (SocialInteractionUtility.IsGoodPositionForInteraction(pawn, friend) && pawn.Position.InHorDistOf(friend.Position, Mathf.CeilToInt(3f))
				...
				else if (!pawn.pather.Moving)
				{
					...
					for (int i = 0; i < 9 && (i != 8 || !intVec.IsValid); i++)
					{
						...
						if (intVec2.InBounds(map) && intVec2.Walkable(map) && intVec2 != pawn.Position && SocialInteractionUtility.IsGoodPositionForInteraction(intVec2, friend.Position, map) && pawn.CanReach(intVec2, PathEndMode.OnCell, Danger.Deadly) && (!intVec.IsValid || pawn.Position.DistanceToSquared(intVec2) < pawn.Position.DistanceToSquared(intVec)))
						{
							intVec = intVec2;
						}
					}
